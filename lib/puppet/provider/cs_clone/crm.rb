...
Puppet::Type.type(:cs_clone).provide(:crm, :parent => Puppet::Provider::Corosync) do
  desc 'Provider to add, delete, manipulate resource clones.'
  ...
  def self.instances
    ...
    doc.root.elements['configuration'].elements['resources'].each_element('clone') do |e|
      items = e.attributes
      clone = { :name => items['id'].to_sym, :metadata => {} }
      if ! e.elements['primitive'].nil?
        clone[:primitive] = e.elements['primitive'].attributes['id']
      elsif ! e.elements['group'].nil?
        clone[:primitive] = e.elements['group'].attributes['id']
      end
      if ! e.elements['meta_attributes'].nil?
        e.elements['meta_attributes'].each_element do |m|
          clone[:metadata][(m.attributes['name'])] = m.attributes['value']
        end
      end
